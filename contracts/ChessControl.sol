 ...
 */
        ...
        )
    {
        ...
        if (moves[moves.length - 1] == accept_draw_const) {
            ...
            require(
                moves[moves.length - 2] == request_draw_const,
            ...
            for (uint256 i = 0; i < moves.length; i++) {
                ...
                require(
                    !checkForCheck(gameState, opponentState),
                    "Invalid check state"
                );
 ...
 */
        ...
        )
    {
        ...
        require(
            ...
            "Invalid move: incorrect piece color"
        );
        ...
        if (fromType == pawn_const) {
            ...
            revert("Invalid move: unsupported piece type");
        }

        // Ensure move is valid
        require(
            newGameState != invalid_move_constant,
            "Invalid move: game state"
        );
    ...
    function verifyExecutePawnMove(
        ...
        if (diff == 8 || diff == 16) {
            ...
            if ((uint8(opponentState & en_passant_const)) != toPos) {
                if (
                    (pieceToPosition == 0) || // Must move to occupied square
                    (currentTurnBlack ==
                        ((pieceToPosition & color_const) == color_const)) // Must capture opponent piece
                ) {
                    return (invalid_move_constant, 0x0);
                }
            }
 ...
 */
    function verifyExecuteRookMove(
        ...
        uint8 v = getVerticalMovement(fromPos, toPos);
        if (
            ((h > 0) == (v > 0)) || // Cannot move both horizontally and vertically
            (gameState & getInBetweenMask(fromPos, toPos)) != 0x00 // Path must be clear
        ) {
            return invalid_move_constant;
        }
 ...
 */
    function verifyExecuteQueenMove(
        ...
        uint8 v = getVerticalMovement(fromPos, toPos);
        if (
            ((h != v) && (h != 0) && (v != 0)) || // Not a valid straight or diagonal move
            (gameState & getInBetweenMask(fromPos, toPos)) != 0x00 // Path must be clear
        ) {
            return invalid_move_constant;
        }
 ...
 */
    ...
    function checkPawnValidMoves(
        ...
        int8[4] memory pawnMoves = currentTurnBlack
            ? [int8(-7), int8(-8), int8(-9), int8(-16)]
            : [int8(7), int8(8), int8(9), int8(16)];
    ...
    function checkKingValidMoves(
        ...
        for (uint8 i = 0; i < kingMoves.length; i++) {
            ...
            if (toPos >= 0 && toPos < 64) {
                ...
                if (
                    (newGameState != invalid_move_constant) &&
                    (!pieceUnderAttack(newGameState, uint8(toPos)))
                ) {
                    return true;
                }
            }
 ...
 */
    function checkEndgame(
        ...
        uint8 kingPiece = uint8(
            (gameState >> (kingPosition << piece_pos_shift_bit)) & 0xF
        );
 ...
 */
    ...
    function checkForCheck(
        ...
        assert(
            king_const ==
                (pieceAtPosition(gameState, kingsPosition) & type_mask_const)
        );
            ...
            while (true) {
                ...
                if (
                    ...
                    ((directions[i] == 1 || directions[i] == -1) &&
                        (uint8(currPos) >> 3) != (pos >> 3))
                ) {
                    break;
                }
                uint8 currPiece = uint8(
                    gameState >> (uint8(currPos) * piece_bit_size)
                ) & 0xF;
                if (currPiece > 0) {
                    if (
                        currPiece == enemyRook ||
        ...
        for (uint8 i = 0; i < directions.length; i++) {
            ...
            while (true) {
                ...
                if (
                    currPos < 0 ||
                    currPos >= 64 ||
                    ((directions[i] == 9 || directions[i] == -9) &&
                        ((uint8(currPos) & 0x7) != (pos & 0x7)))
                ) {
                    break;
                }
                uint8 currPiece = uint8(
    ...
    function checkKnightAttack(
        ...
        for (uint8 i = 0; i < knightMoves.length; i++) {
            ...
            if (
                currPos >= 0 &&
                currPos < 64 &&
                ((gameState >> (uint8(currPos) * piece_bit_size)) & 0xF) ==
                enemyKnight
            ) {
                return true;
            }
